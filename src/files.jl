export post_ac_opf, post_acr_opf, post_soc_opf, post_qc_opf, post_dc_opf
...
"""
Given a JuMP model and a PowerModels network data structure,
Builds an AC-OPF formulation of the given data in RECTANGULAR COORDINATES and returns the JuMP model
"""
function post_acr_opf(data::Dict{String,Any}, model=Model())
    @assert !haskey(data, "multinetwork")
    @assert !haskey(data, "conductors")
