export poly_acr_opf, poly_acr_opf_deg4, smitha_constraint_deg4
"""
...
function poly_acr_opf_deg4(data::Dict{String,Any}, model=PolyModel())
    ...
    for (i,branch) in ref[:branch]
        ...
        # AC Line Flow Constraints
        p[f_idx] = @NLexpression model (g+g_fr)/tm^2*(vr_fr^2 + vi_fr^2) + (-g*tr+b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) + (-b*tr-g*ti)/tm^2*(vi_fr*vr_to - vr_fr*vi_to) 
        q[f_idx] = @NLexpression model -(b+b_fr)/tm^2*(vr_fr^2 + vi_fr^2) - (-b*tr-g*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) + (-g*tr+b*ti)/tm^2*(vi_fr*vr_to - vr_fr*vi_to) 
        
        p[t_idx] = @NLexpression model  (g+g_to)*(vr_to^2 + vi_to^2) + (-g*tr-b*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) + (-b*tr+g*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to)) 
        q[t_idx] = @NLexpression model -(b+b_to)*(vr_to^2 + vi_to^2) - (-b*tr+g*ti)/tm^2*(vr_fr*vr_to + vi_fr*vi_to) + (-g*tr-b*ti)/tm^2*(-(vi_fr*vr_to - vr_fr*vi_to)) 
        ...
        @NLconstraint(model, -ref[:branch][l]["rate_a"] <= (p[(l,i,j) in ref[:arcs]]) <= ref[:branch][l]["rate_a"])
        @NLconstraint(model, -ref[:branch][l]["rate_a"] <= (q[(l,i,j) in ref[:arcs]]) <= ref[:branch][l]["rate_a"])
    ...
    for (i, bus) in ref[:bus]
        ...
        if length(ref[:bus_gens][i])>1 || !(ref[:bus_gens][i]==i) 
            error("generators do not match buses")
        end

        pg[i] = @NLexpression model sum(p[a] for a in ref[:bus_arcs][i]) +
                sum(p_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) +
                sum(load["pd"] for load in bus_loads) +
                sum(shunt["gs"] for shunt in bus_shunts)*(vr[i]^2+vi[i]^2)
        
        qg[i] = @NLexpression model sum(q[a] for a in ref[:bus_arcs][i]) +
                sum(q_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) +
                sum(load["qd"] for load in bus_loads) -
                sum(shunt["bs"] for shunt in bus_shunts)*(vr[i]^2+vi[i]^2)
...
function smitha_constraint_deg4(data, model)
    ref = PowerModels.build_ref(data)[:nw][0]
